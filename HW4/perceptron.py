        ...
        for a in range(2): #iterates over the algorithms
            ...
            #--> add your Python code here
               if clf.predict([x_testSample]) == y_testSample:
                  ...
                  clfFalse += 1
            if a == 0:
               perScore = clfTrue / (clfTrue + clfFalse)
               if perHigh < perScore:
                  perHigh = perScore
               print(f"Highest Perceptron accuracy so far: {perHigh}, Parameters: learning rate=0.01, shuffle=True")
            else:
               mlpScore = clfTrue / (clfTrue + clfFalse)
               if mlpHigh < mlpScore:
                  mlpHigh = mlpScore
               print(f"Highest MLP accuracy so far: {mlpHigh}, Parameters: learning rate=0.02, shuffle=False")
